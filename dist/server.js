...
export function buildServer(ctx) {
    const server = new McpServer({ name: SERVER_NAME, version: SERVER_VERSION }, {
        instructions: 'System Task — gestão de tarefas, projetos e equipe. Comece por systemtask_day_brief (o seu ' +
    ...
    for (const tool of TOOLS) {
        server.registerTool(tool.name, {
            ...
            try {
                return { content: [{ type: 'text', text: truncate(await tool.run(ctx, args ?? {})) }] };
            }
            catch (e) {
                ...
                return { content: [{ type: 'text', text: `Não deu: ${msg}` }], isError: true };
            }
        });
    }
    return server;
