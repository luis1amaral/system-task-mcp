...
export const tasksSearch = {
    ...
    description: 'Lista tarefas filtrando por projeto, responsável, situação e período. Para saber COMO um ' +
    ...
    async run({ api }, a) {
        ...
        if (a.unassigned)
            params.push('unassigned=1');
        if (a.completed !== undefined)
            params.push(`completed=${a.completed ? 1 : 0}`);
        if (a.from)
            params.push(`from=${a.from}`);
        if (a.to)
            params.push(`to=${a.to}`);
        const limit = Math.min(a.limit ?? 25, 50);
        params.push(`limit=${limit}`);
        const r = await api.get(`/api/tasks?${params.join('&')}`);
        if (r.tasks.length === 0)
            return 'Nenhuma tarefa com esses filtros.';
        // Assignee names only resolve inside a list; with no project, fall back to the id.
        const names = new Map();
        if (categoryId != null)
            for (const m of await api.members(categoryId))
...
export const taskCreate = {
    ...
    description: 'Cria uma tarefa — sua, ou já no nome de alguém do projeto (assignee). Uma tarefa tem UM ' +
    ...
    inputSchema: {
        ...
        assignee: z
            .string()
            .optional()
            .describe('Nome de usuário ou id de quem assume. Exige "project" — é dentro dele que o nome é ' +
...
export const taskUpdate = {
    ...
    async run({ api }, a) {
        ...
        if (a.estimateMin !== undefined)
            body.durationMin = a.estimateMin;
        if (a.description !== undefined)
            body.description = a.description;
        if (Object.keys(body).length === 0)
            return 'Você não disse o que mudar na tarefa.';
...
export const taskAssign = {
    ...
    description: 'UMA pessoa é responsável por uma tarefa, ou nenhuma — não existem dois. Marcar outra TROCA: ' +
...
export const taskMove = {
    ...
    description: 'Muda a tarefa de projeto. Se ela for RECORRENTE, a série inteira vai junto por padrão — mover ' +
    ...
    async run({ api }, a) {
        ...
        const r = await api.post(`/api/tasks/${a.task}/move`, {
            categoryId,
...
export const taskComment = {
    ...
    description: 'Escreve um comentário na tarefa — é como o agente devolve resultado para a equipe, e o que ' +
...
export const taskGet = {
    ...
    description: 'A tarefa COMPLETA: título e descrição sem corte, mais as subtarefas. Use sempre que o título ' +
    ...
    readOnly: true,
    async run({ api }, a) {
        const r = await api.get(`/api/tasks/${a.task}`);
        const t = r.task;
        const parts = [
            `## #${t.id} — ${t.title}`,
        ...
        if (r.steps?.length) {
            ...
            for (const s of r.steps)
                parts.push(`- [${s.completed ? 'x' : ' '}] ${s.description}`);
        }
        if (t.tags?.length)
            parts.push('', `Etiquetas: ${t.tags.join(', ')}`);
        return parts.join('\n');
    },
};
