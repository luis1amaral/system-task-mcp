...
export const whoami: ToolDef = {
  ...
  description:
    'Confirma a conta conectada e o fuso horário usado nos relatórios. Use uma vez no começo se ' +
    'houver dúvida sobre qual conta o token abre, ou se outra tool falhar por autenticação.',
  inputSchema: {},
  readOnly: true,
  async run({ api }) {
    const r = await api.get<{ profile: any }>('/api/me');
    const p = r.profile;
    return [
...
export const projectsList: ToolDef = {
  name: 'systemtask_projects_list',
  title: 'Meus projetos e como cada um está',
  description:
    'Visão geral de TODOS os projetos que você enxerga, com progresso, atrasadas, sem responsável e ' +
  ...
  readOnly: true,
  async run({ api }) {
    const r = await api.get<{ today: string; tz: string; projects: any[] }>('/api/reports/projects');
    if (r.projects.length === 0) return 'Você ainda não tem nenhum projeto no System Task.';
...
export const projectReport: ToolDef = {
  name: 'systemtask_project_report',
  title: 'Painel de um projeto',
  description:
    'Como um projeto está: total, concluídas, abertas, atrasadas, distribuição por coluna do board ' +
  ...
  async run({ api }, a) {
    ...
    const r = await api.get<{ report: any }>(
      `/api/reports/projects/${project.id}${query({ from: a.from, to: a.to })}`,
    );
    const rep = r.report;
    const t = rep.totals;
    const parts: string[] = [
    ...
    if (rep.byColumn?.length) {
      parts.push('', '**Por coluna (abertas)**', table(['Coluna', 'Abertas'], rep.byColumn.map((c: any) => [c.name, c.open])));
    }
    if (rep.byPriority?.length > 1 || rep.byPriority?.some((p: any) => p.priority !== 'sem')) {
      parts.push('', '**Por prioridade (abertas)**', table(['Prioridade', 'Abertas'], rep.byPriority.map((p: any) => [p.priority, p.open])));
    }
    if (rep.throughput?.length) {
      parts.push('', `**Conclusões por dia:** ${rep.throughput.map((d: any) => `${fmtDate(d.date)}=${d.done}`).join(' · ')}`);
    }
    return parts.join('\n');
  },
};

export const projectRisks: ToolDef = {
  name: 'systemtask_project_risks',
  title: 'Onde está o risco de um projeto',
  description:
    'O que vai dar problema: tarefas atrasadas, sem data e sem responsável — com as mais antigas ' +
  ...
  async run({ api }, a) {
    const project = await api.resolveProject(a.project);
    const r = await api.get<{ today: string; attention: any }>(
      `/api/reports/projects/${project.id}/attention${query({ sample: a.sample })}`,
    );
    ...
    if (at.sample.length) {
      parts.push(
        '',
        table(
          ['#', 'Tarefa', 'Vence', 'Atraso', 'Responsável'],
          at.sample.map((t: any) => [
            `#${t.id}`,
            cut(t.title, 40),
            fmtDate(t.date),
...
export const teamList: ToolDef = {
  name: 'systemtask_team_list',
  title: 'Quem participa de um projeto',
  description:
    'Os membros do projeto, com o nome de usuário. Use antes de atribuir uma demanda quando não ' +
  ...
  async run({ api }, a) {
    ...
    return table(
      ['Pessoa', 'Usuário', 'Papel'],
      members.map((m) => [m.displayName || m.username, `@${m.username}`, m.role ?? 'editor']),
    );
  },
};

export const teamLoad: ToolDef = {
  name: 'systemtask_team_load',
  title: 'Carga da equipe e quem está sem demanda',
  description:
    'Quanto cada pessoa tem em aberto, quanto atrasou, quanto concluiu no período e quantas horas ' +
  ...
  async run({ api }, a) {
    const project = await api.resolveProject(a.project);
    const r = await api.get<{ period: any; members: any[]; idle: any[] }>(
      `/api/reports/projects/${project.id}/members${query({ from: a.from, to: a.to })}`,
    );
    const parts = [
      ...
      '',
      table(
        ['Pessoa', 'Abertas', 'Atrasadas', 'Concluídas', 'Estimado'],
        r.members.map((m) => [
    ...
    parts.push(
      '',
      r.idle.length
        ? `**Sem demanda nenhuma:** ${r.idle.map((m: any) => `@${m.username}`).join(', ')}`
...
export const dayBrief: ToolDef = {
  name: 'systemtask_day_brief',
  title: 'Meu dia',
  description:
    'O que EU tenho para hoje: o que vence hoje, o que já está atrasado, quanto tempo estimado ' +
    'isso soma, e o backlog sem data. Traz só o que é meu — tarefa atribuída a outra pessoa é o ' +
    'dia dela. Comece o dia por aqui.',
  inputSchema: {},
  readOnly: true,
  async run({ api }) {
    const r = await api.get<{ day: any }>('/api/reports/me/day');
    const d = r.day;
    const parts = [
