...
export const listCreate: ToolDef = {
  name: 'systemtask_list_create',
  title: 'Criar uma lista',
  description:
    'Cria uma lista nova (o app chama isto de "Lista"; as tools de tarefa mais antigas chamam a ' +
...
export const listRename: ToolDef = {
  name: 'systemtask_list_rename',
  title: 'Renomear (ou recolorir) uma lista',
  description:
    'Muda o nome, a cor ou o emoji de uma lista já existente — as três coisas passam pela mesma ' +
  ...
  async run({ api }, a) {
    ...
    if (Object.keys(body).length === 0) return 'Você não disse o que mudar na lista (nome, cor ou emoji).';
...
export const listDelete: ToolDef = {
  name: 'systemtask_list_delete',
  title: 'Apagar uma lista (vai para a lixeira, reversível)',
  description:
    'Manda a lista para a LIXEIRA — não apaga nada de fato. As tarefas dela vão junto e ficam ' +
...
export const listRestore: ToolDef = {
  name: 'systemtask_list_restore',
  title: 'Restaurar uma lista da lixeira',
  description:
    'Tira a lista da lixeira e devolve as tarefas dela junto — o vínculo nunca se perdeu. Use ' +
...
export const listPurge: ToolDef = {
  name: 'systemtask_list_purge',
  title: 'Apagar uma lista PARA SEMPRE (sem lixeira, sem volta)',
  description:
    'Remove a lista de vez — sem passar pela lixeira, sem systemtask_list_restore depois. Use isto ' +
  ...
  inputSchema: {
    ...
    confirmRequest: z
      ...
      .max(500)
      .describe(
        'O pedido da PESSOA, nas palavras dela, pedindo a remoção definitiva — não uma frase sua. ' +
