...
export function cut(s: string | null | undefined, n = 40): string {
 ...
 */
export function fmtDate(iso: string | null | undefined, currentYear = String(new Date().getFullYear())): string {
  if (!iso) return '—';
...
export function table(headers: string[], rows: (string | number)[][]): string {
  if (rows.length === 0) return '_(nada aqui)_';
  ...
  for (const r of kept) out.push(`| ${r.map((x) => String(x ?? '')).join(' | ')} |`);
  return out.join('\n');
}
